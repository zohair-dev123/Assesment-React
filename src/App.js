...
function App({ value }) {
  ...
  const handleButtonClick = () => {
    ...
    apiResponse.forEach((item) => {
      document.getElementById(`checkbox-${item.id}`).checked = false; // Uncheck all checkboxes
    });
    ...
    if (radioValue === "all") {
      ...
      while (randomIndexes.size < Math.min(3, apiResponse.length)) {
        randomIndexes.add(Math.floor(Math.random() * apiResponse.length));
      }
      randomIndexes.forEach((index) => {
        const item = apiResponse[index];
        document.getElementById(`checkbox-${item.id}`).checked = true;
        setSelected((prevSelected) => [...prevSelected, item]);
      });
    }
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <div className="w-3/5 bg-white py-10 px-7">
        Add Text
        <div className="flex items-center justify-center mt-4">
          ...
          <input
            type="text"
            placeholder=""
            className="w-44 text-sm bg-transparent border-[1.5px] px-5 py-3 rounded mb-4 ml-4 outline-none"
            value={null}
            onChange={(e) => {
              console.log("CHANGE");
            }}
        ...
        <div class="flex items-center mb-4">
          <input
            ...
            value="all"
            name="default-radio"
            className="w-4 h-4 text-orange-700 bg-gray-100 border-gray-300 focus:ring-orange-700   dark:bg-orange-700 dark:border-orange-700"
        ...
        <div className="w-80 flex items-center px-4 bg-slate-100 rounded-md">
          <FaMagnifyingGlass className="text-slate-500 cursor-pointer hover:text-black mr-2" />
          <input
            type="text"
        ...
        <button
          className="absolute bottom-4 right-1/4 bg-orange-600 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded"
          onClick={handleButtonClick}
        >
          Apply text to Item(s)
        </button>
      </div>
    </div>
  );
